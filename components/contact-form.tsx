...
export default function ContactForm() {
  ...
  useEffect(() => {
    if (state?.status === 'success') {
      toast.success(state.message,{position: 'top-right', className: 'bg-green-500 text-white'})
    }
    if (state?.status === 'error') {
      toast.error(state.message,{position: 'top-right', className: 'bg-red-500 text-white'})
    }
  }, [state])
